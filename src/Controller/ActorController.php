...
class ActorController extends AbstractController
{
    ...
    public function edit($actor_id, Request $request)
    {
        $repository = $this->getDoctrine()
            ->getRepository(Actor::class);
        $actor = $repository->find($actor_id);
        /*$original_occupations = $actor->getOccupations();
        $original_places = $actor->getPlaces();
        $test = count($original_occupations);
        $test = count($original_places);*/
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // but, the original `$actor` variable has also been updated
            ...
            foreach($actor->getOccupations() as $occupation){
                echo $occupation->getOccupation()."<br>";
                $occupation->setStartDateByString($occupation->getTextStartDate());
                $occupation->setEndDateByString($occupation->getTextEndDate());
            }
            if(isset($original_occupations)) {
                foreach ($original_occupations as $occupation) {
                    if (!$actor->getOccupations()->contains($occupation)) {
            ...
            if(isset($original_places)) {
                foreach ($original_places as $place) {
                    if (!$actor->getPlaces()->contains($place)) {
            ...
            foreach($actor->getPrimaryRelationships() as $relationship){
                if($relationship->getTextStartDate(true) === ""){
                    $relationship->setStartDate(null);
                    $relationship->setStartDateAccuracy(null);
                }
                if($relationship->getTextEndDate(true) === "") {
                    $relationship->setEndDate(null);
                    $relationship->setEndDateAccuracy(null);
                }
    ...
    public function autocompleteAction(Request $request)
    {
        ...
        $qb = $em->getRepository(Actor::class)->createQueryBuilder('c');
        $first = true;
        foreach($terms as $searchTerm) {
            if($first) {
                $qb->where('c.first_name LIKE :name OR c.surname LIKE :name OR YEAR(c.birthdate) LIKE :name OR YEAR(c.date_of_death) LIKE :name')
                    ->setParameter('name', '%' . $searchTerm . '%');
                $first = false;
            }
            else{
                $qb->andWhere('c.first_name LIKE :name OR c.surname LIKE :name OR YEAR(c.birthdate) LIKE :name OR YEAR(c.date_of_death) LIKE :name')
                    ->setParameter('name', '%' . $searchTerm . '%');
            }
        }
        ...
        foreach ($entities as $entity)
        {
            ...
            $newRow['name'] = $entity->getFirstName() . " " . $entity->getSurname();
            $newRow['dates'] = ' ('.$entity->getBirthYear()."&#8211;".$entity->getDeathYear().")";
            $names[] = $newRow;
        }
